...
hyperparameters_source = {
    ...
    'max_grad_norm': 0.5,
    'vf_coef': 0.5,
    'tensorboard_log': 'ppo_custom_hopper_source_tensorboard/'
}

def train_sourcemodel(env_id):
    train_env = gym.make(env_id)
    print('State space:', train_env.observation_space)  # state-space
    print('Action space:', train_env.action_space)  # action-space
    print('Dynamics parameters:', train_env.get_parameters())  # masses of each link of the Hopper
    env = make_vec_env(env_id, n_envs=4)
    model = PPO("MlpPolicy", env, verbose=1, **hyperparameters_source)
    model.learn(total_timesteps=1000000)
    model.save("model_ppo")
    return model

def train_targetmodel(env_id):
    train_env = gym.make(env_id)
    print('State space:', train_env.observation_space)  # state-space
    print('Action space:', train_env.action_space)  # action-space
    print('Dynamics parameters:', train_env.get_parameters())  # masses of each link of the Hopper
...
def main():
    ...
    source_model = train_sourcemodel(source_env_id)
    ...
    target_model = train_targetmodel(target_env_id)
